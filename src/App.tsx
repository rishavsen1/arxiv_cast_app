...
export default function App() {
  ...
  const getSubcategoryFullForm = (topic: string, sub: string) => {
    ...
    return (
      SUBCATEGORY_FULL_FORMS[fullKey] ||
      `Full form not available for ${fullKey}`
    );
  };
  ...
  const handleGeneratePodcast = async () => {
    ...
    try {
      ...
      const { generatePodcastScript, generatePodcastAudio } =
        await import("./podcast");
  ...
  const toggleLiveMode = async () => {
    if (isLiveMode) {
      ...
      const papersContext = selectedPapersData
        .map((p) => `Title: ${p.title}\nAbstract: ${p.abstract}`)
        .join("\n\n");
  ...
  return (
    <div className="min-h-screen bg-[#f9f8f6] text-[#2c2c2a] flex flex-col font-sans relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
          <div className="bg-white border border-[#d5d3cb] rounded-xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-4 border-b border-[#e5e3db] shrink-0">
              <h3 className="text-lg font-semibold text-[#2c2c2a] font-serif">
                {modal.title}
              </h3>
              <button
                onClick={closeModal}
                className="text-[#5c5c5a] hover:text-black transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      <header className="bg-white border-b border-[#e5e3db] p-4 flex justify-between items-center z-10">
        ...
        <a
          href="#"
          className="text-sm text-[#5c5c5a] hover:text-[#5a5a58] transition-colors"
        >
          Return to Dashboard
        </a>
      </header>
          ...
          <aside className="w-full lg:w-96 bg-white border-r border-[#e5e3db] flex flex-col shrink-0 overflow-y-auto">
            ...
            <div className="border-b border-[#e5e3db] shrink-0">
              ...
              >
                ...
                {isPodcastStudioOpen ? (
                  <ChevronUp className="w-5 h-5 text-[#5c5c5a]" />
                ) : (
                  <ChevronDown className="w-5 h-5 text-[#5c5c5a]" />
                )}
              </button>
                ...
                <div className="p-4 pt-0 space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <label className="text-xs text-[#5c5c5a] uppercase">
                        Style
                      </label>
                      <select
                        value={podcastStyle}
                  ...
                  <div className="bg-[#f9f8f6]/50 rounded-lg p-3 border border-[#e5e3db]/50">
                    <h4 className="text-xs font-semibold text-[#4a4a48] uppercase mb-2">
                      Voice Models
                    </h4>
                    <div className="space-y-2 text-xs text-[#5c5c5a]">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono bg-[#f0eee6] px-1.5 py-0.5 rounded text-[10px]">
                          Puck (gemini-2.5-flash-preview-tts)
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="flex items-center gap-1">
                        ...
                        <span className="font-mono bg-[#f0eee6] px-1.5 py-0.5 rounded text-[10px]">
                          Kore (gemini-2.5-flash-preview-tts)
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="border-b border-[#e5e3db] flex flex-col min-h-0 shrink-0">
              ...
              {isTranscriptOpen && (
                <div
                  className="overflow-auto p-4 text-sm text-[#4a4a48] space-y-4 prose prose-invert prose-p:leading-relaxed max-h-96"
            ...
            <div className="border-b border-[#e5e3db] flex flex-col min-h-0 shrink-0">
              <div className="w-full p-4 flex justify-between items-center bg-white/80 sticky top-0">
                ...
                >
                  ...
                  <h3 className="text-sm font-semibold text-[#4a4a48] uppercase tracking-wider font-serif">
                    Discuss Papers
                  </h3>
                </button>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {chatMessages.length === 0 ? (
                      <div className="text-[#7a7a78] text-sm text-center mt-10">
                        Ask a question about the {selectedPapers.size} selected
                        papers.
                      </div>
                    ) : (
                      chatMessages.map((msg, i) => (
                        ...
                        >
                          <div
                            className={`max-w-[85%] rounded-lg p-3 text-sm ${msg.role === "user" ? "bg-[#3a3a38] text-white" : "bg-[#f0eee6] text-[#2c2c2a]"}`}
                          >
                            {msg.text}
                          </div>
                        </div>
                    ...
                    {isLiveMode && (
                      <>
                        ...
                        <div
                          ...
                          style={{
                            transform: `scale(${1 + liveVolume * 0.8})`,
                          }}
                        />
                      </>
                    )}
            ...
            <div className="flex flex-col bg-[#f9f8f6] shrink-0">
              ...
              {isVaultOpen && (
                <div className="overflow-auto p-2 space-y-1 max-h-64">
                  {archives.length === 0 ? (
                    <div className="text-xs text-[#7a7a78] p-2 text-center">
                      No archives found.
                    </div>
                  ) : (
                    archives.map((file) => (
          ...
          <div className="bg-white/50 p-4 border-b border-[#e5e3db] shrink-0">
            <div className="flex flex-wrap gap-6 items-start">
              <div className="space-y-2 flex-1">
                <h3 className="text-sm font-semibold text-[#5c5c5a] uppercase tracking-wider font-serif">
                  Categories
                </h3>
                      ...
                      <div className="text-xs font-bold text-[#7a7a78] uppercase flex items-center gap-1">
                        ...
                        <button
                          ...
                          onClick={() =>
                            showAlert(
                              topic.toUpperCase(),
                              getCategorySubcategoryList(topic, subjects),
                            )
                          }
                      ...
                      <div className="flex flex-wrap gap-2">
                        {subjects.map((sub) => {
                          ...
                          return (
                            <label
                              key={cat}
                              className="flex items-center gap-1 text-sm cursor-pointer hover:text-[#5a5a58]"
                            >
                              <input
                                type="checkbox"
              ...
              <div className="space-y-4 flex-1 min-w-[200px]">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <label className="text-sm text-[#4a4a48]">
                      Papers per tag:
                    </label>
                    <input
                      type="number"
                      value={papersPerTag}
                      onChange={(e) =>
                        setPapersPerTag(parseInt(e.target.value) || 1)
                      }
          ...
          <div className="flex-1 overflow-auto p-4">
            {papers.length === 0 ? (
              <div className="h-full flex items-center justify-center text-[#7a7a78]">
                No papers loaded. Select categories and click "Search &
                Populate".
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm divide-y divide-[#e5e3db]">
                  {papers.map((paper) => (
                    <tr
                      key={`${paper.id}-${paper.category}`}
                      className="hover:bg-[#f5f4ef] transition-colors"
                    >
                      <td className="p-2 align-top">
                        <button
                          onClick={() => togglePaperSelection(paper.id)}
                          className="text-[#7a7a78] hover:text-[#3a3a38]"
                        >
                      ...
                      <td className="p-2 align-top">
                        <div className="flex flex-wrap gap-1">
                          {paper.other_categories ? (
                            paper.other_categories.split(",").map((c) => (
                              <span
                                key={c}
                                className="inline-block px-1.5 py-0.5 bg-[#f0eee6] text-[#5c5c5a] rounded text-[10px] font-mono"
                              >
                                {c}
                              </span>
                            ))
                      ...
                      <td className="p-2 align-top">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="font-medium text-[#2c2c2a] hover:text-[#3a3a38] hover:underline line-clamp-2"
                        >
                          {paper.title}
                        </a>
                        <div className="text-xs text-[#7a7a78] mt-1">
                          {paper.date}
                        </div>
                      </td>
